...
for batch in batches:
    for plate in plates:
        ...
        if all_plates_df.shape[0]==0:
            all_plates_df = df.copy()
        else:
            frames = [all_plates_df, df]
            all_plates_df = pd.concat(frames, ignore_index=True, join="inner")

fs_all_plates_df = feature_select(profiles=all_plates_df,
                                  features="infer",
...
for batch in batches:
    for plate in plates:
        df = fs_all_plates_df.query('Metadata_batch==@batch').reset_index(drop=True)
        df = df.drop(columns=['Metadata_batch'])
        df.to_csv(f'profiles/{batch}/{plate}/{plate}_normalized_feature_select_all_plates_negcon.csv.gz', index=False)
